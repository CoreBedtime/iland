/*
 * drm_linux.c — Linux DRM API stubs for macOS / libwayland-mac.
 *
 * Presents a single virtual DRM device (fd = DRM_VIRTUAL_FD) with:
 *   - one CRTC  (id 1)
 *   - one encoder (id 1, attached to CRTC 1)
 *   - one connected DisplayPort connector (id 1) at 1920×1080@60
 *
 * Mode-set and page-flip operations are forwarded to framebufferd via
 * drm_send_json().  All other ioctls return -1 / ENOSYS.
 */

#include "drm_linux.h"
#include "drm.h"       /* drm_send_json */
...
#define DRM_VIRTUAL_FD  42   /* sentinel; never a real file descriptor */
...
int drmGetCap(int fd, uint64_t capability, uint64_t *value)
{
    ...
    case DRM_CAP_PRIME:               *value = 3; break; /* import + export */
...
drmModeEncoderPtr drmModeGetEncoder(int fd, uint32_t encoder_id)
{
    ...
    e->encoder_id     = 1;
    e->encoder_type   = 10; /* TMDS — DisplayPort */
    e->crtc_id        = 1;
...
/* Simple monotonic FB id allocator */
...
{
    (void)pixel_format; (void)bo_handles; (void)pitches;
    (void)offsets; (void)flags;
    return drmModeAddFB(fd, width, height, 24, 32, pitches ? pitches[0] : 0,
                        bo_handles ? bo_handles[0] : 0, buf_id);
}
                    ...
                    uint32_t flags, void *user_data)
{
    ...
    /* Synthesise an immediate page-flip completion if caller wants an event */
    (void)user_data;
    return 0;
}
...
{
    /* No real kernel events — page flips are synchronous on the virtual device.
     * Return 0 so callers polling this fd don't spin. */
    (void)fd; (void)evctx;
    return 0;
}
...
/* Dumb buffers are backed by anonymous memory on this platform. */
                            ...
                            uint64_t *size)
{
    ...
    uint32_t p  = (width * ((bpp + 7) / 8) + 63) & ~63u; /* 64-byte aligned */
    uint64_t sz = (uint64_t)p * height;
    ...
    void *map = mmap(NULL, sz, PROT_READ | PROT_WRITE,
                     MAP_PRIVATE | MAP_ANON, -1, 0);
    if (map == MAP_FAILED) return -1;

    uint32_t h = g_next_dumb_handle++;
    g_dumb[slot] = (dumb_buf_t){ h, sz, map };
...
int drmModeDestroyDumbBuffer(int fd, uint32_t handle)
{
    ...
    for (int i = 0; i < MAX_DUMB_BUFS; i++) {
        if (g_dumb[i].handle == handle) {
            munmap(g_dumb[i].map, g_dumb[i].size);
...
int drmModeMapDumbBuffer(int fd, uint32_t handle, uint64_t *offset)
{
    ...
    for (int i = 0; i < MAX_DUMB_BUFS; i++) {
        if (g_dumb[i].handle == handle) {
            /* Encode slot as the mmap offset; caller uses mmap(fd, offset).
             * Since we already have a real mapping, return its address cast
             * as offset — callers that mmap(DRM_VIRTUAL_FD, offset) will get
             * EBADF, but compositors using dumb bufs typically use the pointer
             * returned here via a second mmap.  Return 0 and let the caller
             * use MAP_SHARED on the anonymous mapping they have. */
...
int drmPrimeHandleToFD(int fd, uint32_t handle, uint32_t flags, int *prime_fd)
{
    ...
    /* PRIME export not implemented — return ENOSYS */
